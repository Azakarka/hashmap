...
/* check custom hash functions */
    void hash_check() {
        ...
        for (int i = 0; i < 1000; ++i) {
            assert(stupid_map.find(i) == stupid_map.end());
            auto old = stupid_map.size();
            stupid_map[i] = i + 1;
            auto current = stupid_map.size();
            assert(old + 1 == current);
            if (stupid_hash_fn(i))
                fail("wrong hash function in class");
        }
...
void assert_map_equal(HashMap<std::string, int> map, std::unordered_map<std::string, int> orig_map) {
    std::vector<std::pair<std::string, int>> vec1, vec2;
...
void run_debug() {
    ...
    using HashMapIterator = HashMap<std::string, int>::iterator;
    std::unordered_map<std::string, int> orig_map;
    for (int i = 0; i < n; ++i) {
        char code;
        std::string key;
        std::cin >> code;
        ...
        if (code == '-') {
            ...
            auto it2 = orig_map.find(key);
            assert((it == map.end() ? -1 : it->second) == ((it2 == orig_map.end() ? -1 : it2->second)));
            // std::cout << (it == map.end() ? -1 : it->second) << "\n";
            assert_map_equal(map, orig_map);
        } else if (code == '+') {
            int val;
            std::cin >> val;
            map[key] = val;
